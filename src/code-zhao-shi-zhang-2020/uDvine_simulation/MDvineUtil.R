...
bootstrapLongDvine <- function(totalfam, totalpara, loc, pastOb, B, eps) {
  totalTree <- length(totalfam) #p
  simuTs <- rep(0,B) #store the simulated one-day ahead predicted time series
  err <- runif(B)
  for(i in 1:B){
    simuTs[i] <- invPredConCDF(err[i], pastOb=pastOb, fam=totalfam, para=totalpara, loc=loc, eps=eps)
  }
  simuTs
}
